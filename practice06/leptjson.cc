...
value::value(const value& v) 
    :type(v.type), sz(v.sz), context(v.context) { }

value::value(value&& v) 
    :type(std::move(v.type)), sz(std::move(v.sz)),
    context(std::move(v.context)) { }

value::value(double i): type(LEPT_NUMBER), 
    sz(0), context(make_shared<double>(i)) { } // json number 构造函数

//value::value(bool b) :type(b|LEPT_FALSE), 
//    sz(0), context(make_shared<bool>(b)) { }   // json bool 构造函数

value::value(const string& s) :type(LEPT_STRING),
    sz(0), context(make_shared<string>(s)) { } // json string 构造函数
...
const value& value::operator=(value&& rhs) {
...
value::value(int t, size_t size, shared_ptr<void> c):type(t), sz(size), context(std::move(c)) 
{ }
...
size_t value::size() { // 返回对象的存储元素个数，仅在json object 和 array 可用
  return sz;
}

value& value::operator[](size_t i) { // 返回元素，仅在 array 可用
  assert(gettype() == LEPT_ARRAY);
  vector<value>& v = *static_cast<vector<value>*>(context.get()); // 注意，是引用类型！
  return v[i];
  //vector<value>* v = static_cast<vector<value>*>(context.get());
  //return (*v)[i];
}

value value::operator[](size_t i) const { // 返回const元素，仅在 array 可用
  assert(type == LEPT_ARRAY && i < sz);
  vector<value>& v = *static_cast<vector<value>*>(context.get());
  return static_cast<value>(v[i]);
}

value& value::operator[](const string& key) { // 字典索引
  assert(type == LEPT_OBJECT);
  map<string, value>& m = *static_cast<map<string, value>*>(context.get());
...
value  value::operator[](const string& key) const {
  ...
  map<string, value>& m = *static_cast<map<string, value>*>(context.get());
  assert(m.count(key) != 0);
  return static_cast<value>(m[key]);
}
      ...
      case COLON: parse_whitespace(json);
                  if(pos == json.size())
                  {
                    ...
                    if(json[pos] == '}')
                    {
                      ++pos;
                      jerrno = LEPT_PARSE_OK;
                      return value(LEPT_OBJECT, obj.size(), make_shared<map<string, value>>(obj));
                    }
                    if(json[pos] == ',')
                    {
                      stat = COMMA;
...
value leptjson::parse_array(const string& json)
{
  ...
  while(1)
  {
    ...
    if(json[pos] == ',')
      ...
      ++pos;
      jerrno = LEPT_PARSE_OK;
      return value(LEPT_ARRAY, a.size(), make_shared<vector<value>>(vector<value>(a)));
      //return value(LEPT_ARRAY, make_shared<vector<value>>(a), a.size());
    }else {
      jerrno = LEPT_PARSE_MISS_COMMA;
      return value(LEPT_NULL_PTR);
    }
  ...
  { 
    switch(json[pos])
    {
      case '\"':
        ...
        return value(LEPT_STRING, 0, make_shared<string>(s));
...
*/
  ...
  if(json[size-1] != '.' && (NUM0TO9(json[pos]) || json[pos] == '-'))
  {
    ...
    if(errno == ERANGE && (n == HUGE_VAL || n == -HUGE_VAL)) // 数值太大，上溢
      ...
      return value(LEPT_NUMBER, 0, make_shared<double>(n));
    }
  }else
    jerrno = LEPT_PARSE_INVALID_VALUE;
  return value(LEPT_NULL_PTR);
...
value  leptjson::parse_literal(const string& json, const string& literal, int type)
{
  ...
  return value(type, 0, make_shared<bool>(b));
}
